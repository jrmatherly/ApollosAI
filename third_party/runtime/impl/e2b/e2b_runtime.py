...
class E2BRuntime(ActionExecutionClient):
            ...
            if self.attach_to_existing and self.sandbox is None:
                try:
                    ...
                    if cached_sandbox_id:
                        try:
                            self.sandbox = E2BBox(
                                self.config.sandbox, sandbox_id=cached_sandbox_id
                            )
                            logger.info(
                                f"Successfully attached to existing E2B sandbox: {cached_sandbox_id}"
                            )
                        except Exception as e:
                            logger.warning(
                                f"Failed to connect to cached sandbox {cached_sandbox_id}: {e}"
                            )
                    ...
                    logger.warning(
                        f"Failed to attach to existing sandbox: {e}. Will create a new one."
                    )
            ...
            if workspace_dir:
                try:
                    ...
                    if exit_code == 0:
                        ...
                        logger.warning(
                            f"Failed to create workspace directory: {output}"
                        )
                except Exception as e:
        ...
        if self.sandbox:
            try:
                if not self.attach_to_existing:
                    ...
                    logger.info(
                        "E2B runtime connection closed, sandbox kept running for reuse"
                    )
    ...
    def run(self, action: CmdRunAction) -> Observation:
        ...
        try:
            ...
            return CmdOutputObservation(
                content=output, command=action.command, exit_code=exit_code
            )
        except Exception as e:
    ...
    def run_ipython(self, action: IPythonRunCellAction) -> Observation:
        ...
        try:
            ...
            return IPythonRunCellObservation(
                content="\n".join(outputs) if outputs else "", code=action.code
            )
        except Exception as e:
    ...
    def read(self, action: FileReadAction) -> Observation:
        if self.file_store is None:
            return ErrorObservation(
                "E2B file store not initialized. Call connect() first."
            )
    ...
    def edit(self, action: FileEditAction) -> Observation:
        ...
        try:
            ...
            if action.start < 0 or action.end > len(lines):
                return ErrorObservation(
                    f"Invalid line range: {action.start}-{action.end}"
                )

            new_lines = (
                lines[: action.start] + action.content.split("\n") + lines[action.end :]
            )
            new_content = "\n".join(new_lines)
    ...
    def list_files(self, path: str | None = None) -> list[str]:
        ...
        try:
            exit_code, output = self.sandbox.execute(
                f"find {path} -maxdepth 1 -type f -o -type d"
            )
            if exit_code == 0:
                files = [
                ...
                return [
                    f.replace(path + "/", "") if f.startswith(path + "/") else f
                    for f in files
                ]
            else:
    ...
    def get_mcp_config(self, extra_stdio_servers: list | None = None) -> dict:
        ...
        return {"stdio_servers": extra_stdio_servers or []}
