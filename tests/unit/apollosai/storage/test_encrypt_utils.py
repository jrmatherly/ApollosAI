...
def _set_encryption_key(monkeypatch):
    ...
    monkeypatch.setenv(
        'APOLLOSAI_ENCRYPTION_KEY', 'test-key-for-unit-tests-must-be-32chars!'
    )
...
def test_decrypt_invalid_raises():
    ...
    with pytest.raises(
        (ValueError, cryptography.exceptions.InvalidTag, binascii.Error)
    ):
...
def test_missing_key_raises(monkeypatch):
    ...
    from apollosai.storage.encrypt_utils import encrypt_value, reset_key_cache
