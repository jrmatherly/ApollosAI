...
class ProcessMonitor {
  ...
  getMacProcessInfo(callback) {
    ...
    exec(
      "ps -eo pid,ppid,command,%mem,%cpu -r | head -n 11",
      (error, stdout) => {
        if (error) {
          callback(error, null);
          return;
  ...
  getWindowsProcessInfo(callback) {
    ...
    exec(
      "wmic process get ProcessId,ParentProcessId,CommandLine,WorkingSetSize /format:csv",
      (error, stdout) => {
        if (error) {
          callback(error, null);
          return;
        ...
        const dataLines = lines.filter(
          (line) => line.trim() !== "" && !line.includes("Node,"),
        );
